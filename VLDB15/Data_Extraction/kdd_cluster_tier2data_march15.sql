...
--------------------------------------------------------------------------------------------------------
--create table kdd_tier2_mar15 as
with population as
...
, (12- first_value(timestamp_hr) over (partition by icustay_id order by timestamp_hr asc)) as vent_duration_12hr
from vent_group1
)
...
join mimic2v26.medevents med on med.icustay_id=pop.icustay_id 
  ...
  and med.charttime <= pop.icustay_intime+12/24
where med.charttime is not null
)

--select * from vaso_group1;
--select distinct itemid, count(*) from vaso_group group by itemid;

, vaso_group as
(select distinct icustay_id
, first_value(value1) over (partition by icustay_id order by value1 desc) as vaso_max_dosage_12hr
from vaso_group1
)
...
, sedative_group as
(select distinct icustay_id
, 12- first_value(timestamp_hr) over (partition by icustay_id order by timestamp_hr asc) as sedative_duration_12hr
...
, coalesce(vent.vent_duration_12hr, 0) as vent_duration_12hr
, coalesce(vaso.vaso_max_dosage_12hr, 0) as vaso_max_dosage_12hr
, coalesce(se.sedative_duration_12hr, 0) as sedative_duration_12hr
